...
<script>
  ...
  import { getdriverSWiper } from "@/plugins/http";
export default {
  data: () => ({
    SwiperIndex: null,
    pageIndex:1,
 ...
 async mounted() {
  //  await this.$axios
  //     .get(
  //       `http://127.0.0.1:8080/api/driverdetail/getdriverSWiper/?driverlastName=${this.$route.query.driverlastName}`
  //     )
  //     .then((res) => {
    const res = await getdriverSWiper({driverlastName:this.$route.query.driverlastName});
        if (res.status === 200) {
