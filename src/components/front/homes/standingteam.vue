<template>
  <div class="race_standing">
    <div class="race_standing_header" >
      <div
        class="race_standing_header_driver"
        @click="$router.push('/home/Standingdriver')"
      >
        车手
      </div>
      <div
        class="race_standing_header_team"
        @click="$router.push('/home/standingteam')"
      >
        车队
      </div>
    </div>
    <v-progress-circular
      ...
      indeterminate
      v-if="isloading"
      class="standing_loding"
    ></v-progress-circular>
    <div class="race_standing_header_title" >
      <div class="race_standing_header_title_container">
        <div class="race_standing_header_title_content" v-if="isshow">
          2022 车队排行榜
        </div>
      </div>
    </div>
    <div class="race_standing_driver" v-if="isshow">
      <div class="race_standing_driver">
        ...
        <img
          ...
          :src="standingteaminfo[1].team_detail_icon"
        />
        <img
          class="race_standing_driver_pic"
          :src="standingteaminfo[1].car_pic"
        />
      </div>
      <div class="race_standing_driver race_standing_driver_middle">
        ...
        <img
          class="race_standing_drivernumber"
          :src="standingteaminfo[0].team_detail_icon"
        />
        <img
          class="race_standing_driver_pic race_standing_driver_pic_middle"
          :src="standingteaminfo[0].car_pic"
        />
      </div>
      <div class="race_standing_driver">
        <div class="race_standing_driver_top"></div>
        <img
          class="race_standing_drivernumber"
          :src="standingteaminfo[2].team_detail_icon"
        />
        <img
          class="race_standing_driver_pic"
          :src="standingteaminfo[2].car_pic"
        />
      </div>
    </div>
    <!-- table -->
    <div class="race_standing_table_container" v-if="isshow">
      <div
        ...
        v-for="(item, index) in standingteaminfo"
        :key="index"
      >
        <div class="race_standing_table_left">
          ...
          <div
            ...
            :style="{ background: `${standingteaminfo[index].team_color}` }"
          ></div>
          <div class="race_standing_firstname">
            {{ standingteaminfo[index].team }}
          </div>

          <div class="race_standing_teamname">
            {{
              standingteaminfo[index].driver_one +
              "/" +
              standingteaminfo[index].driver_one
            }}
          </div>
        ...
        <div class="race_standing_table_right">
          <div class="race_standing_points">
            {{ standingteaminfo[index].points + " PTS" }}
          </div>
  ...
  <script>
  import { getStandingteam } from "@/plugins/http";
export default {
  data: () => ({
    isshow: false,
    isloading:true,
 ...
 async mounted() {
    // this.$axios
    //   .get("http://127.0.0.1:8080/api/home/getStandingteam")
    //   .then((res) => {
      const res = await getStandingteam();
        if (res.status === 200) {
          for (let key in res.data) {
            this.standingteaminfo.push(res.data[key]);
          }
